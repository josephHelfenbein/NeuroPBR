...
fragment float4 pbr_fragment(
    ...
    float3 rayDir = normalize((frame.cameraToWorld * float4(rayDirCam, 0.0)).xyz);
    float3 rayOrigin = frame.cameraPosFov.xyz;
    ...
    if (discriminant < 0.0) {
        // Miss: Render Environment
        color = envMap.sample(linearSampler, rayDir).rgb * frame.iblParams.x;
    } else {
        // Hit
        ...
        if (t < 0.0) {
            ...
            float3 N = normalize(P - sphereCenter);

            // UV Mapping (Spherical)
            float u = 0.5 + atan2(N.z, N.x) / (2.0 * kPi);
            float v = 0.5 - asin(N.y) / kPi;
            ...
            color = (diffuse * (1.0 - metallic) + specular) * frame.iblParams.x;
